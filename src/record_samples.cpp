...
videoRecorder::videoRecorder(const int writerCount, const string baseName_,
                const Size imageSize_, double fps, bool isColor,
                const string outputPath_,  bool isMultiProcess_, bool isTimeStampRequired_)
...
// isMultiProcess_ - should multi-threading / multiprocessing be used to speed up the process
{
  ...
  isTimeStampRequired = isTimeStampRequired_;
...
void videoRecorder::writeFrames(const vector<Mat>& newFrames)
{
  ...
  if(isMultiProcess)
  {
    ...
    if(isFinished)
    {
      ...
      {
        string timeStamp = return_time_header();
        for(auto& frame:newFrames)
          {

            putText(frame, timeStamp, Point(0, 40), 
            	FONT_HERSHEY_COMPLEX_SMALL, 2, Scalar(0, 255, 0), 2);
...
stitchedVideoRecorder::stitchedVideoRecorder(const int writerCount, const string baseName,
					    ...
					    bool isTimeStampRequired):
			                    ...
			                    (writerCount, baseName, imageSize, fps,
                                             isColor, outputPath, isMultiProcess, isTimeStampRequired)
   ...
   // use multiple threads for accomplishing the stitching and writing process
  {
       ...
       stitchedImages.size() > 0)
    {
      ...
      if(isTimeStampRequired)
      {
          ...
          string timeStamp = return_time_header();
          putText(imageTowrite, timeStamp, Point(0, 80), 
            	  FONT_HERSHEY_COMPLEX_SMALL, 4, Scalar(255, 255, 255), 5);
      }
